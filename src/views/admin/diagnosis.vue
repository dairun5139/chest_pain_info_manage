  ...
  <div>
    <el-container style="padding: 0px;margin:0 0 0 0">
      ...
      <el-aside width="20%" style="padding:0 5px 0 0;background:white;border-style: dotted;border-width: 0px 1px 0px 0px;border-color:#C0C0C0;">
        ...
        <el-table ref="singleTable" v-loading="catload" stripe max-height="660" style="margin-left:5px;margin-top:0px" highlight-current-row :data="menus.filter(data => !search || data.name.toLowerCase().includes(search.toLowerCase()))" @current-change="handleCurrentChange">
          ...
          <el-table-column align="right">
            ...
            <template slot>
              <el-button v-show="iseditCat" type="text" size="mini" @click="dialogVisible=true;catchange.name=currentRow.name;dialogType='edit';handleCurrentChange(row)"><i class="el-icon-edit" /></el-button>
              <el-button v-show="iseditCat" type="text" size="mini" @click="delDmsDiscat"><i class="el-icon-delete" /></el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-aside>
      ...
      <el-main style="margin-left:10px">
        ...
        <el-button v-if="currentRow.name!=='请点击左侧选择诊断类别'" type="text" @click="dialogVisibleDis=true;dialogType='add';onedis.catId=currentRow.id;onedis={}"><i class="el-icon-circle-plus" />新增诊断</el-button>
        <el-table v-loading="disload" :data="dis" style="margin-top:30px">
          <el-table-column label="疾病编码" property="code" />
          <el-table-column label="疾病名称" property="name" />
          <el-table-column label="ICD编码" property="icd" />
    ...
    <el-dialog :visible.sync="dialogVisible" width="500px" :title="dialogType==='edit'?'修改诊断分类目录':'新增诊断分类目录'" @close="getDeplist">
      <el-form>
        <el-form-item label="诊断分类目录名称:">
          <el-input v-model.lazy="catchange.name" style="width:300px" />
        </el-form-item>
      </el-form>
    ...
    <el-dialog :visible.sync="dialogVisibleDis" width="500px" :title="dialogType==='edit'?'修改疾病信息':'新增疾病信息'" @close="getDeplist">
      <el-form :model="onedis">
        <el-form-item label="疾病编码:">
          <el-input v-model="onedis.code" style="width:300px" />
        </el-form-item>
        <el-form-item label="疾病名称:">
          <el-input v-model="onedis.name" style="width:300px" />
        </el-form-item>
        <el-form-item label="ICD编码:">
          <el-input v-model="onedis.icd" style="width:300px" />
        </el-form-item>
      </el-form>
      <div style="text-align:right;margin-top:20px">
